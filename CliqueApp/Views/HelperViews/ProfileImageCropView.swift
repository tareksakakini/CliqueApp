...
struct ProfileImageCropView: View {
    ...
    // Constants for better UX - circular crop for profile pictures
    private let cropSize = CGSize(width: 300, height: 300) // Square for circular crop
                            ...
                            .gesture(
                                SimultaneousGesture(
                                    ...
                                    MagnificationGesture()
                                        ...
                                        .onEnded { _ in
                                            lastScale = scale
                                            // Re-constrain offset after scale change
    ...
    private func cropImage() {
        let imageSize = image.size
        let outputSize = CGSize(width: 640, height: 640) // Square output for profile pictures
        ...
        guard let cgImage = image.cgImage?.cropping(to: clampedCropRect) else {
            // Fallback
            let fallbackImage = resizeImageToSquare(image, to: outputSize)
            onCrop(fallbackImage)
            return
        }
        
        let croppedImage = UIImage(cgImage: cgImage, scale: image.scale, orientation: image.imageOrientation)
        let finalImage = resizeImageToSquare(croppedImage, to: outputSize)
        
        onCrop(finalImage)
    }
    
    private func resizeImageToSquare(_ image: UIImage, to targetSize: CGSize) -> UIImage {
